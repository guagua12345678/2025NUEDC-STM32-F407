...
void Emm_V5_Reset_CurPos_To_Zero(uint8_t addr)
{
  ...
  // 发送命令
  if(addr == 1) USART2_DMA_Send((uint8_t *)cmd, 4);
  if(addr == 2) USART3_DMA_Send((uint8_t *)cmd, 4);
}
...
void Emm_V5_Read_Sys_Params(uint8_t addr, SysParams_t s)
{
  ...
  // 发送命令
  if(addr == 1) USART2_DMA_Send((uint8_t *)cmd, i);
  if(addr == 2) USART3_DMA_Send((uint8_t *)cmd, i);
}
...
void Emm_V5_Modify_Ctrl_Mode(uint8_t addr, uint8_t svF, uint8_t ctrl_mode)
{
  ...
  // 发送命令
  if(addr == 1) USART2_DMA_Send((uint8_t *)cmd, 6);
  if(addr == 2) USART3_DMA_Send((uint8_t *)cmd, 6);
}
...
void Emm_V5_Vel_Control(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, uint8_t snF)
{
  ...
  // 发送命令
  if(addr == 1) USART2_DMA_Send((uint8_t *)cmd, 8);
  if(addr == 2) USART3_DMA_Send((uint8_t *)cmd, 8);
}
...
void Emm_V5_Pos_Control(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, uint32_t clk, uint8_t raF, uint8_t snF)
{
  ...
  // 发送命令
  if(addr == 1) USART2_DMA_Send((uint8_t *)cmd, 13);
  if(addr == 2) USART3_DMA_Send((uint8_t *)cmd, 13);
}
...
void Emm_V5_Stop_Now(uint8_t addr, uint8_t snF)
{
  ...
  // 发送命令
  if(addr == 1) USART2_DMA_Send((uint8_t *)cmd, 5);
  if(addr == 2) USART3_DMA_Send((uint8_t *)cmd, 5);
}
...
void Emm_V5_Origin_Modify_Params(uint8_t addr, uint8_t svF, uint8_t o_mode, uint8_t o_dir, uint16_t o_vel, uint32_t o_tm, uint16_t sl_vel, uint16_t sl_ma, uint16_t sl_ms, uint8_t potF)
{
  ...
  // 发送命令
  if(addr == 1) USART2_DMA_Send((uint8_t *)cmd, 20);
  if(addr == 2) USART3_DMA_Send((uint8_t *)cmd, 20);
}
